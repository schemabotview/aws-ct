...
- **13 — HA, DR, Cost & Migration (10)** → `aws-cloud` scene (cross-cutting; frames the closest anchors: HA §2 → `region-a` (`asg`/`aurora-a`/`aurora-b`), DR/Backup §3–4 → `region-b` (`glacier`/`s3-serving`), cost §5–7 → `global-plane` (`cost`/`trusted-advisor`), migration §8–9 → `region-a` `ec2-a` / `region-b` `s3-raw`).
- **14 — Well-Architected & SAA Exam Prep (11)** → `aws-cloud` scene (synthesis; each pillar frames its closest anchor: Ops §2 → `global-plane` (`cloudformation`/`cloudwatch`), Security §3 → `cross-cut` (`kms`/`secrets`), Reliability §4 → `region-a` (`asg`/`aurora`), Performance §5 → `region-a` (`ec2-a`/`cache-a`), Cost/Sustainability §6–7 → `global-plane` `cost`, tools §8 → `trusted-advisor`; the exam-prep recap sections §1/§9/§10/§11 are `role: hook` = whole-map).
