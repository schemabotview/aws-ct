...
**Status:** structure drafted — **14 modules, 148 sections** (spine below). **All 14 modules authored (148 sections)** — every section has `.ipynb` + `.slide` + `.tts`, and `manifest.json` wires them (01 → `aws-global`, 02 → `aws-iam`, 03–14 → `aws-cloud`, framed per section via `focus`/`highlight`). All three scenes are ported into the graphl-movie app (monogram tiles + official AWS service icons). Pending: the remaining Colab `.wav`s (97 of 148 generated). Intended remote: `github.com/schemabotview/aws-ct`.
